...
const TaskDetails = () => {
  ...
  // const truestatus = async()=>{
  //   try {
  //     const res = await ApiService.GetGeneraltrueStatus({
  //       taskItemId
  //     })
  //     const datas = res.data.subCategoryStatusWithFalseStatus
  //     setLengthoftrue(datas)
  ...
  //console.log('lengthoftrue',lengthoftrue);
  useEffect(() => {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (response && response.status === 200) {
        ...
        const dataLng = res.data?.subCategoryStatusWithFalseStatus?.length;
        if (dataLng === 0 || serviceName == "Rodent Pro") {
